...
ObjectFactory.registerObjectType(
  'Scenario',
  (baseObject) => {
    return function ScenarioObject(data,context) {
      _(this).extend(new baseObject(data,context))
      this.speak = () => "I'm a scenario object"
      this.name = data.name
...
function CallsAPIStep(step,ctx){
  ...
  this.validate = () => {
    return [  should(!!ctx.exists("Component",step.called_by),`Component ${step.called_by} doesn't exist`),
              should(!!(ctx.exists("ExternalService",step.api) || ctx.exists("Component",step.api)),`ExternalService or Component ${step.api} doesn't exist`)
           ]
  }
}
