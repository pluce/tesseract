...
//console.log(data)
    ...
    render_data = {
      ...
      custom_doc_files: _(model.custom_doc.files()).without('index.md')
    }
...
function overloadPages(model) {
  return function(workdir) {
    context=model.context
    return fse.stat(path.join(model.path,'docs'))
    .then(stats => {
      if(stats.isDirectory()){
        return exec('cp -r '+path.join(model.path,'docs','*')+' '+path.join(workdir,'docs'))
      } else {
        return false
      }
    }) 
